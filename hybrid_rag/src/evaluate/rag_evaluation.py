...
class RAGAEvaluator:
        ...
        try:
            ...
            for column_name in ["contexts"]:
                if column_name in ds.features:
                    ...
                    if not (isinstance(feature, Sequence) and feature.feature.dtype == "string"):
                        self.logger.error(
            ...
            self.logger.error(f"Failed to validate dataset columns: {traceback.format_exc()}")
            return "FAIL"
    ...
    def evaluate_rag(self, question: List[str], answer: List[str], context: List[Document]) -> dict:
        """
        ...
        try:
            ...
            self.logger.info("Successfully evaluated the questions, answers, and context.")
            return result

        except Exception as e:
            error = str(e)
            self.logger.error(f"Failed to evaluate RAG: {error} -> TRACEBACK: {traceback.format_exc()}")
            raise
