...
class MongoCredentialManager:
    ...
    def __get_creds_from_mongo(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            error = str(e)
            logger.error(
                f"Failed to Retrieve the Data from MongoDB Reason: {error} -> TRACEBACK : {traceback.format_exc()}"
            )
            raise
    ...
    def _format_creds(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Successfully Created the Dictionary of MogoCreds from Collection : {self.collection_name} and DB: {self.db_name}"
            )
        except Exception as e:
            error = str(e)
            logger.error(
                f"Failed to Create the Dictionary of Mongocreds Reason: {error} -> TRACEBACK : {traceback.format_exc()}"
            )
        return mongo_dict
